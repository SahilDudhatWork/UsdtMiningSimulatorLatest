...
export const AuthProvider = ({ children }) => {
    ...
    try {
      ...
      if (storedSession) {
        ...
        if (
          ...
          sessionData.apiResponse.data.user
        ) {
  ...
  const login = async (email, password) => {
    try {
      const res = await axios.post('https://peradox.in/api/mtc/login', {
        email,
        password,
      });
  ...
  const signup = async (name, email, password, refer_code) => {
    try {
      ...
      const res = await axios.post(
        'https://peradox.in/api/mtc/register',
        payload,
      );
  ...
  const logout = async () => {
    try {
      ...
      const userDataParse = sessionUserData
