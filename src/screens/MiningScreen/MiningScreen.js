...
const MiningScreen = props => {
  ...
  const checkCooldownStates = async () => {
    try {
      ...
      const timeBoostCooldown = await AsyncStorage.getItem(
        'timeBoostAdCooldown',
      );
  ...
  const checkMysteryBoxAvailability = async () => {
    try {
      const lastMysteryBoxTime = await AsyncStorage.getItem(
        'lastMysteryBoxTime',
      );
      ...
      const referenceTime = lastMysteryBoxTime
        ? parseInt(lastMysteryBoxTime, 10)
        : parseInt(firstLaunchTime, 10);
